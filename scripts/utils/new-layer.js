add_new_layer = (args, layers_count, framework) => {
    ...
    let translate = {};
    translate["Convolution 2D"] = "2D卷积层";
    translate["Max Pool 2D"] = "2D池化层";
    translate["Convolution 3D"] = "3D卷积层";
    translate["Max Pool 3D"] = "3D池化层";
    translate["Dense"] = "全连接层";
    translate["Output"] = "输出层";
    translate["Activation"] = "激活函数";
    translate["Flatten"] = "展平";
    translate["Batch Normalization"] = "批标准化";

    layerName = (args.name in translate) ? translate[args.name] : args.name;
    ...
    layer.innerHTML = `<svg class="add" id="layer-${layers_count}-add" xmlns="http://www.w3.org/2000/svg" onclick="add_new_layer_buttons(this)">
                        ...
                        <label class="main-heading-4">${layerName}</label>
                        ...
                        <br>
                        <p class="info">卷积核个数: 32</p>
                        <p class="info">卷积核大小: 3,3</p>
                        <p class="info">步长: 1</p>
                        <p class="info">激活函数: relu</p>
                        <p class="info">填充值: valid</p>`

    //changing the information.
    info = layer.getElementsByClassName('info');
    switch (args.name) {
        case "Convolution 1D":
            info[1].innerHTML = "卷积核大小: 3";
            new_layer = {
        ...
        case "GRU":
            ...
            info[2].innerHTML = "Recurrent 激活函数: sigmoid"
            info[1].innerHTML = "激活函数: tanh"
            info[0].innerHTML = "神经元数目：10"
            new_layer = {
                id: `layer-${layers_count}`,
                name: args.name,
